...
// ---------- Зберігання історії (реалізація - history.h, FFat ring-buffer'и) ----------
// Це лише термін зберігання (днів), який користувач може змінити через
// /settings/history - не впливає на розмір файлів на флеші (вони завжди
// розраховані на HISTORY_RETENTION_DAYS_MAX із history.h), лише на те, з
// якого моменту дані вважаються дійсними при читанні для графіків.
...
// ---------- Deep sleep при відсутності живлення на PSU in ТА UPS out ----------
// Плата живиться від акумулятора через окремий DC-DC (QS-1205CME, див.
// схему) - незалежно від того, є напруга на PSU in чи UPS out. Тобто ESP32
// продовжує працювати навіть коли обидва канали "мертві" - саме цей стан і
// означає "система зараз простоює" (мережі немає, навантаження вимкнене
// або від'єднане) і варта того, щоб заснути заради акумулятора.
//
// PV-пін INA3221 (GPIO4) дає апаратний "швидкий шлях" пробудження, АЛЕ з
// важливим обмеженням: Power-Valid - це AND по ВСІХ ТРЬОХ каналах одразу
// (апаратна особливість чипа, не наше рішення - Upper/Lower Limit це єдина
// пара регістрів на всі 3 канали). Battery практично завжди вище порогу
// (акумулятор же й живить плату), тож на практиці PV=HIGH фактично означає
// "PSU in І UPS out ОБИДВА повернулись" - а не "АБО", як хотілося б.
// Тому є друге, таймерне джерело пробудження (SLEEP_TIMER_RECHECK_US) -
// справжня OR-логіка ("чи повернулось хоч щось") перевіряється в софті
// після кожного пробудження, з реальних свіжих показів. PV лишається як
// оптимізація швидкості реакції для типового випадку (мережа повертається -
// UPS одразу починає передавати напругу на вихід, тож обидва канали
// зазвичай зʼявляються майже одночасно).
#define SLEEP_VOLTAGE_THRESHOLD_V     2.0f   // нижче цього на PSU in ТА на UPS out одночасно - вважаємо "немає напруги"
#define SLEEP_PV_UPPER_V              3.0f   // апаратний поріг Power-Valid Upper-Limit - трохи вище SLEEP_VOLTAGE_THRESHOLD_V для запасу/гістерезису
#define SLEEP_PV_LOWER_V              2.0f   // апаратний поріг Power-Valid Lower-Limit
#define SLEEP_PV_GPIO                 GPIO_NUM_4

// Скільки часу умова "немає напруги на обох" має тривати БЕЗПЕРЕРВНО перед
// першим засинанням - захист від флапання на короткочасних просіданнях/
// перехідних процесах (напр. під час перезавантаження NAS). 2 хвилини -
// достатньо, щоб не спрацювати на щось короткочасне, і водночас не
// затягувати з реальним переходом у простій.
#define SLEEP_DEBOUNCE_MS             (2UL * 60 * 1000)

// Скільки чекати між "підстрахувальними" пробудженнями по таймеру, поки
// спимо - це те, що фактично реалізує "АБО" (див. коментар вище): якщо
// апаратний PV чомусь не спрацював (напр. повернувся лише один з двох
// каналів), пристрій все одно прокинеться сам і перевірить свіжі покази не
// пізніше, ніж через цей інтервал.
#define SLEEP_TIMER_RECHECK_US        (3ULL * 60 * 1000000ULL)

// Коротка "перевірка й одразу назад спати" затримка для пробуджень САМЕ по
// таймеру (коли умова й досі "немає напруги") - на відміну від повного
// SLEEP_DEBOUNCE_MS: тут не потрібно чекати довго, бо ми вже й так довго
// спали в стані "нічого не змінилось", повторний повний дебаунс лише
// даремно тримав би WiFi/сервер увімкненими на 2 хвилини щоцикл.
#define SLEEP_TIMER_RECHECK_DEBOUNCE_MS (5UL * 1000)
