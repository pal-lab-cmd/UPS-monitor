...
- Reads all 3 INA3221 channels (bus voltage, shunt voltage, current, power) with per-channel shunt resistance + zero offset calibration (`config.h → CH_CAL`)
...
- Web UI (`/`) — voltage/current/power table per channel, refreshed every second, **UK/EN language switch**
...
- Single access password (username is always `admin`) for WiFi settings, reboot and OTA; changeable from the UI
...
5. Change the default access password via `/wifi` ("Access password" section) — default is `change-me`, set in `config.h → AUTH_DEFAULT_PASS`
...
| `/api/data` | GET | none | JSON with current channel readings (telemetry — intentionally open) |
| `/wifi` | GET | yes | WiFi and access password settings form |
...
Auth is HTTP Basic (username `admin`, password from `Preferences`/`config.h`).
...
| `ups.status` | `OL`/`OB` based on voltage on the `PSU in` channel (`INPUT_PRESENT_V` threshold), `LB` is added when `battery.voltage` < `BATTERY_LOW_V` |
| `battery.voltage`, `battery.current` | `Battery` channel |
| `battery.charge` | **estimate** based on battery voltage (linear interpolation between `BATTERY_EMPTY_V` and `BATTERY_FULL_V`) |
| `input.voltage` | `PSU in` channel |
| `output.voltage`, `output.current` | `UPS out` channel |
| `ups.power`/`ups.realpower`, `ups.load` | computed from the `UPS out` channel, `ups.load` — % of `UPS_RATED_POWER_W` |

**`battery.charge` limitation:** LiFePO4 has a very flat discharge curve, so estimating charge from voltage alone is imprecise in the middle of the range (~20–90%). A more accurate approach would be coulomb counting (integrating the measured battery current over time) — the hardware already supports this, it's planned.
...
- `battery.charge` — more accurate estimation via coulomb counting instead of voltage-based linear interpolation
...
- No history of readings is stored — only the current state
